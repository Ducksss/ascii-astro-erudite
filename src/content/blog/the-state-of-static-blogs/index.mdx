...
Hello! My name is Chai Pin Zheng ([Ducksss](https://github.com/Ducksss) on GitHub), and I'm a software engineer who has been fiddling with blogging platforms for a couple of years now.
...
- You can specify multiple post authors via frontmatter. If this post author's ID is found within the `Authors` collection, then it will render particular info from that author's frontmatter file, `[author-name].md` (e.g. avatar, link to profile).
